...
int sys_rpg_create_character(int cclass){
	...
	if(p->ptr){
		...
		struct player* character = malloc(sizeof(struct player));
		if(!character){
			// allocation failed
			errno = ENOMEM;
			return ERROR;
...
int sys_rpg_join(pid_t player){
	...
	p->ptr->party_member = true;
	player_task->ptr->party_member = true;
